          ...
          {user ? (
            ...
            <>
              ...
              <Link href="/signup" className="btn-brand text-sm py-2.5 px-4">
                {authEnabled ? "Start free" : "Open studio"}
              </Link>
            </>
          )}
        </div>
        ...
        <div className="md:hidden border-t border-line bg-bg-elev">
          <div className="px-5 py-4 flex flex-col gap-3">
            ...
            <div className="flex gap-2 pt-2">
              {user ? (
                <Link href="/dashboard" className="btn-brand flex-1 text-sm py-2.5">Dashboard</Link>
              ) : (
                <>
...
function UserButton({ user }: { user: SimpleUser }) {
  const initial = (user.name || user.email || "?").trim().charAt(0).toUpperCase();
  return (
    <div className="flex items-center gap-2">
      <Link href="/dashboard" className="text-sm text-ink-dim hover:text-ink px-3 py-2">
        Dashboard
      </Link>
      <Link
